...
function DoctorLayout() {
  ...
  const { usuario, logout } = useAuth(); // 👈 datos desde el contexto
  ...
  const menuItems = [
    ...
    {
      path: '/dashboard/doctor/consultas',
      icon: 'bi bi-heart-pulse',
      label: 'Atender Pacientes',
    },
    {
      path: '/dashboard/doctor/historiales',
      icon: 'bi bi-file-earmark-medical',
      label: 'Historiales',
    },
    {
      path: '/dashboard/doctor/mi-perfil',
      icon: 'bi bi-person',
      label: 'Mi Perfil',
    },
  ];
  ...
  const getPageTitle = () => {
    ...
    return current ? current.label : 'Panel Doctor';
  };

  const cerrarSesion = async () => {
    try {
      await authService.logout(); // notifica al backend
    } catch (error) {
      console.error('Error al cerrar sesión en backend:', error);
    }
    logout(); // limpia contexto y localStorage, redirige a login
    navigate('/login'); // respaldo
  };

  // Nombre del usuario desde el contexto (se actualiza automáticamente)
  const userName = usuario ? `${usuario.nombres} ${usuario.apellidos}` : 'Doctor';
    ...
    <div className="d-flex" style={{ minHeight: '100vh' }}>
      ...
      >
        <div className="p-3 d-flex flex-column h-100">
          ...
          {/* INFORMACIÓN DEL DOCTOR (usando contexto) */}
          {sidebarOpen && (
            <div className="mb-4">
