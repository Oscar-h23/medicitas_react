...
/* ======================================================
   TIPOS – todos los campos viven en el backend
   ====================================================== */
...
interface PerfilData {
  ...
  sexo: string;
  // Campos que antes estaban en localStorage
  direccion: string;
  distrito: string;
...
function MiPerfil() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Incluir contraseña si se escribió algo
      if (password) {
      ...
      // 3. Enviar al backend
      const respuesta = await pacienteService.actualizarPerfil(payload);

      // 4. Actualizar estado local con la respuesta del backend
      const perfilActualizado: PerfilData = {
        ...DEFAULT_PERFIL,
        ...respuesta.usuario, // asumiendo que el backend devuelve usuario con todos los campos
      };
    ...
    try {
      ...
      autoTable(doc, {
        ...
        body: [
          ...
          ['Sexo', perfil.sexo === 'M' ? 'Masculino' : perfil.sexo === 'F' ? 'Femenino' : '-'],
      ...
      autoTable(doc, {
        ...
        body: ((historial as any).consultas || []).map((c: any) => [
          c.fecha || '-',
          c.doctor || '-',
          c.diagnostico || '-',
        ]),
            ...
            {editando ? (
              ...
              <form onSubmit={handleSubmit}>
                <div className="row g-3">
                  ...
                  {/* Sección: información adicional (ahora en backend) */}
